...
bool constant_time_equal(const char* lhs, std::size_t lhs_len,
...
HttpServer::HttpServer(KeyboardEngine& keyboard, const char* api_token,
    ...
    : keyboard_(keyboard), api_token_(api_token),
      status_provider_(status_provider), status_context_(status_context)
{
}
...
bool HttpServer::start()
{
    ...
    const httpd_uri_t* routes[] = {&status_uri, &down_uri, &up_uri, &press_uri,
                                   &release_uri, &combo_uri};
...
bool HttpServer::authorized(httpd_req_t* req) const
{
    ...
    return std::strncmp(header, kBearerPrefix, sizeof(kBearerPrefix) - 1) == 0 &&
           constant_time_equal(header + sizeof(kBearerPrefix) - 1, supplied_length,
                               api_token_, token_length);
}
...
bool HttpServer::read_json(httpd_req_t* req, cJSON*& root) const
{
    ...
    while (received < req->content_len) {
        const int result = httpd_req_recv(req, body.get() + received,
...
bool HttpServer::parse_duration(cJSON* root, uint64_t& duration_ms) const
{
    ...
    if (!cJSON_IsNumber(item) || !std::isfinite(item->valuedouble) ||
        item->valuedouble < 1 || item->valuedouble > kMaxDurationMs ||
...
esp_err_t HttpServer::status(httpd_req_t* req)
{
    ...
    const StatusSnapshot snapshot = status_provider_ ?
        status_provider_(status_context_) : StatusSnapshot{};
    ...
    cJSON_AddNumberToObject(response, "uptime_ms",
                            static_cast<double>(snapshot.uptime_ms));
    char* payload = cJSON_PrintUnformatted(response);
    cJSON_Delete(response);
    if (!payload) {
...
esp_err_t HttpServer::combo(httpd_req_t* req)
{
    ...
    if (!cJSON_IsArray(items) || cJSON_GetArraySize(items) == 0 ||
        ...
        cJSON_Delete(root);
        send_error(req, 400, "invalid_combo", "keys must contain 1 to 6 keys and duration_ms must be 1 to 1000");
        return ESP_OK;
    }
...
bool HttpServer::send_ws_error(httpd_req_t* req, const char* error,
                               const char* message) const
{
    cJSON* response = cJSON_CreateObject();
    if (!response) {
...
bool HttpServer::send_ws_success(httpd_req_t* req) const
{
    return send_ws_json(req, kSuccessJson);
}
...
bool HttpServer::dispatch_websocket_message(httpd_req_t* req, cJSON* root)
{
    ...
    if (std::strcmp(type, "key_down") != 0 && std::strcmp(type, "key_up") != 0) {
        return send_ws_error(req, "unknown_command", "Supported commands are key_down, key_up, release_all");
    }
    KeyCode key{};
...
esp_err_t HttpServer::websocket(httpd_req_t* req)
{
    ...
    if (packet.len > kMaxRequestBody) {
        keyboard_.release_all();
        return send_ws_error(req, "body_too_large", "The WebSocket message is too long") ?
            ESP_OK : ESP_FAIL;
    }
